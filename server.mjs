...
app.get("/api/nearbyStores", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        error: "Error fetching nearby stores from Google",
...
app.post("/api/stores", async (req, res) => {
  try {
    ...
    if (!placeId || !name) {
      return res
        .status(400)
        .json({ message: "Place ID and name are required." });
    }
    ...
    if (error.code === 11000) {
      ...
      return res
        .status(409)
        .json({ message: "Store with this Place ID already exists." });
    }
    res
      .status(500)
      .json({
        message: "Internal server error while saving store to database.",
        error: error.message,
      });
  }
});
...
app.get("/api/stores", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Internal server error while fetching stores from database.",
        error: error.message,
      });
  }
});
...
app.get("/api/stores/nearby", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        message: "Internal server error while fetching nearby stores.",
        error: error.message,
      });
  }
});
...
app.get("/", (req, res) => {
  res.send(
    "Backend is running. Use /api/nearbyStores to access Google Places API. Use /api/stores for your database."
  );
});
